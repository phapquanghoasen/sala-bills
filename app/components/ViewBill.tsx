...
const ViewBill: React.FC<ViewBillProps> = ({ bill, billId, total, onEdit }) => {
  ...
  const handlePrint = async () => {
    ...
    try {
      await addDoc(collection(db, 'printQueue'), {
        ...
        foods: bill.foods,
        total: bill.foods.reduce(
          (sum: number, food: BillFood) => sum + food.price * food.quantity,
          0
        ),
  ...
  return (
    <div>
      ...
      <p className="mb-1 text-sm sm:text-base">
        <b>{BILL_DETAIL_DESC}: </b>
        <span className="font-mono">{bill.description}</span>
      </p>
      ...
      >
        {printLoading ? 'Đang gửi...' : BILL_DETAIL_PRINT}
      </button>
